 ...
 * @author Kevin Wayne
 */
public class WeightedQuickUnionUFByZhao {
	private int[] parent;  // parent[i] = parent of i
	private int[] size;  // parent[i] = parent of i
	 ...
	 */
	public WeightedQuickUnionUFByZhao(int n) {
		parent = new int[n];
		size = new int[n];
		count = n;
		for (int i = 0; i < n; i++) {
	...
	public static void main(String[] args) {
		int n = StdIn.readInt();
		WeightedQuickUnionUFByZhao uf = new WeightedQuickUnionUFByZhao(n);
		while (!StdIn.isEmpty()) {
			int p = StdIn.readInt();
			int q = StdIn.readInt();
